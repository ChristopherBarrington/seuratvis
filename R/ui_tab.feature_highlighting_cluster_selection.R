...
.ui_tab.feature_highlighting_cluster_selection <- function() {
  bquote({
    ...
    boxes <- list(cluster_dim_plot=box(title='Clustered map',
                  ...
                  sel_cluster_dim_plot=box(title='Selected cluster map',
    ...
    menuItem(text='Highlight features & clusters', tabName=NS(namespace=tab, id='tab'), icon=icon('search')) %>% list() %>% append(x=menus) -> menus

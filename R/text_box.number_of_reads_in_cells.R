...
number_of_reads_text_box.ui <- function(id, width=12) {
  sprintf(fmt='### %s-number_of_reads_text_box.ui', id) %>% message()
...
number_of_reads_text_box.server <- function(input, output, session) {
  ...
  renderValueBox(expr={
    ...
    switch(module_env$id,
           cell_filtering=sprintf(fmt='Total UMI remaining (%.1f%%)', n_filtered/n_reference*100),
           'Total UMI in cells') -> subtitle

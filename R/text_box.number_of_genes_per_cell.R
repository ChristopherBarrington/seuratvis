...
#' 
number_of_genes_per_cell_text_box.ui <- function(id, width=12, f=median, f_name=deparse(substitute(f))) {
  sprintf(fmt='### %s-number_of_genes_per_cell_text_box.ui', id) %>% message()

  module <- 'number_of_genes_per_cell_text_box'
...
#' @rdname number_of_genes_per_cell_text_box
#' 
number_of_genes_per_cell_text_box.server <- function(input, output, session, seurat, cell_filtering, ...) {
  session$ns('') %>% sprintf(fmt='### %snumber_of_genes_per_cell_text_box.server') %>% message()

  # get environments containing variables to run/configure this object
  collect_environments(id=parent.frame()$id, module='number_of_genes_per_cell_text_box') # provides `seuratvis_env`, `server_env` and `module_env`

  # make the text box
  renderValueBox(expr={
    # send a message
    session$ns('') %>% sprintf(fmt='### %snumber_of_genes_per_cell_text_box.server-renderValueBox') %>% message()
    ...
    list(value={n_filtered %>% round(digits=1) %>% comma()},
      ...
      do.call(what=valueBox)}) -> output$number_of_genes_per_cell_text_box
}
